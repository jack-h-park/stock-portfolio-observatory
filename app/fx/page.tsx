    ...
    <>
      ...
      <Card title={copy.hanaOutbound} className="mb-5" info={copy.hanaOutboundHint}>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricField label={copy.estimatedGl} value={dual(data.summary.hanaOutboundEstimatedUnrealizedKrw, data.summary.hanaOutboundEstimatedUnrealizedKrw / (data.summary.currentUsdKrw || 1))} hint={language === 'ko' ? '직접 환전 추정분 + 토스 미매칭·기타 계좌 입금의 입금일 환율 가정' : 'Direct estimates plus deposit-date assumptions for unmatched Toss and other inbound USD'} tone="warning" valueClassName="text-title" />
          ...
          <MetricField label={language === 'ko' ? '기타 계좌에서 받은 USD' : 'Other inbound USD'} value={formatUsd(data.summary.hanaOtherInboundUsd)} hint={language === 'ko' ? '미래에셋 및 목적지 미표시 입금; 추정 원가에 포함' : 'Mirae Asset and unidentified inbound; included in estimated cost'} valueClassName="text-title" />
        </div>
      </Card>

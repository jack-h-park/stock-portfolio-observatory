...
export function getPortfolioSnapshotSeries(scopeKey: string, days = 3650): PortfolioSnapshotSeries[] {
  const conn = db()
  try {
    const table = conn
    ...
    if (table) {
      const columns = new Set(
        (conn.prepare('pragma table_info(portfolio_snapshot_series)').all() as { name: string }[]).map((column) => column.name)
      )
      const optional = (name: string) => columns.has(name) ? name : `null as ${name}`
      return conn
        .prepare(`
          select snapshot_date, captured_at, scope_key, total_cost, priced_cost,
            market_value, unrealized_gl, return_pct, cost_coverage,
            position_coverage, position_count, priced_position_count,
            ${optional('fx_rate')}, ${optional('fx_as_of_date')}, ${optional('fx_source')}
          from portfolio_snapshot_series
          where scope_key = ? and snapshot_date >= date('now', ?)
          order by snapshot_date
        `)
        .all(scopeKey, `-${Math.max(1, Math.floor(days))} days`) as PortfolioSnapshotSeries[]
    }
  } finally {
    conn.close()
  }

  const legacyScopes: Record<string, {
    cost: keyof PortfolioSnapshot
    value: keyof PortfolioSnapshot
    gain: keyof PortfolioSnapshot
    returnPct: keyof PortfolioSnapshot
    coverage: keyof PortfolioSnapshot
  }> = {
    'portfolio:all': { cost: 'global_base_cost', value: 'global_base_market_value', gain: 'global_base_unrealized_gl', returnPct: 'global_base_return_pct', coverage: 'market_value_coverage' },
    'listing:KR': { cost: 'kr_cost_basis', value: 'kr_market_value', gain: 'kr_unrealized_gl', returnPct: 'kr_return_pct', coverage: 'kr_market_value_coverage' },
    'listing:US': { cost: 'us_cost_basis_base', value: 'us_market_value_base', gain: 'us_unrealized_gl_base', returnPct: 'us_return_pct', coverage: 'us_market_value_coverage' },
    'asset:crypto': { cost: 'crypto_cost_basis_base', value: 'crypto_market_value_base', gain: 'crypto_unrealized_gl_base', returnPct: 'crypto_return_pct', coverage: 'crypto_market_value_coverage' },
  }
  const legacy = legacyScopes[scopeKey]
  const snapshots = getPortfolioSnapshots(days)
  if (!legacy && scopeKey !== 'asset:securities') return []

  return snapshots.map((snapshot) => {
    const totalCost = legacy
      ? Number(snapshot[legacy.cost] ?? 0)
      : Number(snapshot.kr_cost_basis ?? 0) + Number(snapshot.us_cost_basis_base ?? 0)
    const marketValue = legacy
      ? Number(snapshot[legacy.value] ?? 0)
      : Number(snapshot.kr_market_value ?? 0) + Number(snapshot.us_market_value_base ?? 0)
    const unrealizedGl = legacy
      ? Number(snapshot[legacy.gain] ?? 0)
      : Number(snapshot.kr_unrealized_gl ?? 0) + Number(snapshot.us_unrealized_gl_base ?? 0)
    const coverage = legacy
      ? Number(snapshot[legacy.coverage] ?? 0)
      : totalCost > 0
        ? (Number(snapshot.kr_cost_basis ?? 0) * Number(snapshot.kr_market_value_coverage ?? 0)
          + Number(snapshot.us_cost_basis_base ?? 0) * Number(snapshot.us_market_value_coverage ?? 0)) / totalCost
        : 1
    return {
      ...
      priced_cost: totalCost * coverage,
      market_value: marketValue,
      unrealized_gl: unrealizedGl,
      return_pct: legacy ? Number(snapshot[legacy.returnPct] ?? 0) : totalCost * coverage > 0 ? unrealizedGl / (totalCost * coverage) * 100 : null,
      cost_coverage: coverage,
      position_coverage: scopeKey === 'portfolio:all' ? Number(snapshot.position_coverage ?? coverage) : coverage,
      position_count: scopeKey === 'portfolio:all' ? Number(snapshot.holding_count ?? 0) : 0,
      priced_position_count: scopeKey === 'portfolio:all' ? Math.round(Number(snapshot.holding_count ?? 0) * Number(snapshot.position_coverage ?? coverage)) : 0,
      fx_rate: null,
      fx_as_of_date: null,
      fx_source: null,
    }

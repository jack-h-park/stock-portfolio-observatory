...
- US: Chase holdings/tax lots, Merrill holding summary/tax-lot detail, Robinhood holdings/tax lots from `data/robinhood-snapshot.json`, and Chase/Fidelity/Merrill/Robinhood transactions/dividends. **Robinhood no longer needs a holdings export dropped into `us-holdings/`** — the snapshot comes from the Robinhood MCP (`get_equity_positions` + `get_equity_tax_lots`), so it is generated rather than downloaded and lives in `data/` like the Toss snapshot. Nothing on a timer can write it: the MCP answers an agent session, not a shell, so it is regenerated by hand and `robinhood_snapshot_fresh` reports how long ago that was. The three `robinhood-holdings-*.pdf` Gain/Loss reports stay where they are as the only record of those positions before the snapshot existed, and are still parsed as evidence, but they are the fallback rather than the source — and that fallback never passes the freshness check at any age, because emailing customer support is not a refresh path.

...
// The generated tabs are published by hand, and this is what notices when
// nobody has. `publish-sheet.mjs` leaves a receipt per market naming the
// database as-of it wrote from; this compares that against the as-of the
// database holds now.
//
// The receipt is read rather than the sheet on purpose: checking a tab itself
...
// reads. Seven days is the span the Korean tab's last lapse ran to sixteen.
const sheetPublishMaxLagDays = Number(process.env.STOCK_SHEET_MAX_LAG_DAYS
  || process.env.STOCK_KR_SHEET_MAX_LAG_DAYS || 7)
for (const [market, receiptFile] of [['KR', 'kr-sheet-publish.json'], ['US', 'us-sheet-publish.json']]) {
  const receiptPath = process.env[`STOCK_${market}_SHEET_RECEIPT_PATH`]
    || path.join(process.cwd(), 'data', receiptFile)
  ...
  const currentAsOf = holdingRows
    ...
    .sort()
    .pop() || null
  const lagDays = receipt?.dbAsOf && currentAsOf
    ? (Date.parse(`${currentAsOf}T00:00:00Z`) - Date.parse(`${receipt.dbAsOf}T00:00:00Z`)) / 86_400_000
    : null
  check(
    `${market.toLowerCase()}_sheet_publish_fresh`,
    receipt == null || lagDays == null || lagDays <= sheetPublishMaxLagDays,
    receipt == null
      ? `no generated ${market} tab published from this workspace (${receiptPath} absent) — nothing to go stale`
      : lagDays == null
        ? `published from ${receipt.dbAsOf ?? 'an unrecorded as-of'}; no current ${market} as-of to compare against`
        : lagDays <= sheetPublishMaxLagDays
          ? `'${receipt.tab}' published from ${receipt.dbAsOf}, ${lagDays.toFixed(0)}d behind the database`
          : `'${receipt.tab}' still shows ${receipt.dbAsOf} while the database is at ${currentAsOf} ` +
            `(${lagDays.toFixed(0)}d behind, ${receipt.rows} row(s) as published) — run ` +
            `\`pnpm publish:${market.toLowerCase()}-sheet --apply\``,
    'warning'
  )
}
